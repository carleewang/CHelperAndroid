...
//        textSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textsize);//默认大小
    ...
    protected final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new MyInertiaTimerTask(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
    ...
    protected final void onItemSelected() {
        if (onItemSelectedListener != null) {
            postDelayed(new MyOnItemSelectedRunnable(this), 200L);
        }
    }
...
//            int drawRightContentStart = measuredWidth * 5 / 6;
