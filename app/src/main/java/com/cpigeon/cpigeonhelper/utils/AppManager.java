...
//            System.exit(0);
    ...
    /**
     * 结束页面到登录界面，看是否存在，存在就跳转到登录，不存在，新建一个页面
     */

    public void startLogin(Context context) {
        RealmUtils.getInstance().deleteUserAllInfo();//删除所有用户资料

        if (!AppManager.getAppManager().isActivityExist(LoginActivity.class)) {//判断登录页是否在栈内
            Intent intent = new Intent(context, LoginActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
        }

        //结束所有activity 保留登录activity
        AppManager.getAppManager().killAllToLoginActivity(LoginActivity.class);
    }

    /***
     * hl  查看指定的Activity是否存在
     *
     */
