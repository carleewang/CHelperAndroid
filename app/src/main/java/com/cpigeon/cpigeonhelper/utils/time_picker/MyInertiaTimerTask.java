 ...
 * @author 小嵩
 */
final class MyInertiaTimerTask extends TimerTask {

    float a;
    final float velocityY;
    final MyWheelView loopView;

    MyInertiaTimerTask(MyWheelView loopview, float velocityY) {
        super();
        loopView = loopview;
    ...
    public final void run() {
        ...
        if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
            ...
            loopView.handler.sendEmptyMessage(MyMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
