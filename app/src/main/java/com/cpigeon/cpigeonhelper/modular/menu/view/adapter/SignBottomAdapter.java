...
public class SignBottomAdapter extends BaseQuickAdapter<GiftBagInfo, BaseViewHolder> {
    ...
    public SignBottomAdapter(Activity activity, SignPresenter mPresenter, SweetAlertDialog mSweetAlertDialog) {
        ...
        size = ScreenTool.getScreenWidth(MyApplication.getInstance().getBaseContext()) / 4;
        animation = AnimationUtils.loadAnimation(MyApplication.getInstance().getBaseContext(), R.anim.anim_sign_box_rock);
    ...
    protected void convert(BaseViewHolder holder, GiftBagInfo item) {
        ...
        imageView.setLayoutParams(new RelativeLayout.LayoutParams(ScreenTool.dip2px(mContext, ImgSize), ScreenTool.dip2px(mContext, ImgSize)));
        if (item.isChoose()) {
            holder.setImageResource(R.id.img, imgId);
            holder.itemView.setOnClickListener(v -> {

                Log.d("qiandao", "convert: dianji");
