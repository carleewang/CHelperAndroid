...
public class HomeNewActivity extends ToolbarBaseActivity implements BulletinView {
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        setTopRightButton("签到", () -> {
            //签到
            startActivity(new Intent(HomeNewActivity.this, SignActivity.class));
...
//            startActivity(intent);
        ...
        setTopLeftButton(R.mipmap.hone_top_news_red, new OnClickListener() {
            @Override
            public void onClick() {
                if (getTopLeftButton() == R.mipmap.hone_top_news_red) {
                    HomeNewActivity.this.setTopLeftButton(R.mipmap.hone_top_news);
                }
                startActivity(new Intent(HomeNewActivity.this, BulletinActivity.class));
            }
        });

        //登录成功，开始单点登录检查
        SingleLoginService.start(HomeNewActivity.this);//启动服务
    ...
    private void initViewPager() {
        ...
        mViewPager.setAdapter(new HomeFgmtAdapter(HomeNewActivity.this.getSupportFragmentManager()));
        ...
        if (list.size() > 0) {
            if (RealmUtils.getInstance().existBulletinEntity()) {//数据库是否存在公告通知数据
                ...
                if (RealmUtils.getInstance().getBulletinEntityTime().equals(list.get(0).getTime())) {
                    ...
                    //保存的数据与服务器公告信息的数据不同
                    HomeNewActivity.this.setTopLeftButton(R.mipmap.hone_top_news_red);
                }
            } else {
                //数据库不存在公告数据
                HomeNewActivity.this.setTopLeftButton(R.mipmap.hone_top_news_red);
