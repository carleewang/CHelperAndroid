...
public class OverdueOrderFragment extends BaseFragment  {
    ...
    //                        if (listApiResponse.getData().get(i).getStatusname().equals("交易过期")) {
                                    if (listApiResponse.getData().get(i).getDdzt() == 2) {
                            ...
                            errSweetAlertDialog = SweetAlertDialogUtil.showDialog(errSweetAlertDialog, msg, OverdueOrderFragment.this.getActivity(), dialog -> {
                                dialog.dismiss();
                                //跳转到登录页
