...
import com.cpigeon.cpigeonhelper.modular.myserver.xh.wdhy.view.adapter.PenalizeRecordAdapter;
...
public class PenalizeRecordListActivity extends ToolbarBaseActivity {
    ...
    private PenalizeRecordAdapter mAdapter;//
    ...
    protected void setStatusBar() {
        ...
        setTopLeftButton(R.drawable.ic_back, PenalizeRecordListActivity.this::finish);
        mHyUserDetailEntity = (HyUserDetailEntity) getIntent().getSerializableExtra("data");
        if (mHyUserDetailEntity != null) {
            setTitle(String.valueOf(mHyUserDetailEntity.getBasicinfo().getName() + "处罚记录"));
        } else {
            setTitle("处罚记录");
        }
        ...
        if (type.equals("myself") && !mHyUserDetailEntity.getZhuangtai().equals("除名")) {
            setTopRightButton(R.mipmap.top_add, () -> {
                //跳转到添加赛事规程页面
                Intent intent = new Intent(PenalizeRecordListActivity.this, PenalizeRecordAddActivity.class);
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        mMemberPresenter = new MemberPresenter(new MenberViewImpl() {
            ...
            public void getServiceResults(ApiResponse<Object> listApiResponse, String msg, Throwable mThrowable) {
                ...
                try {
                    if (listApiResponse.getErrorCode() == ErrorCodeService.LOG_SERVICE) {
                        errSweetAlertDialog = SweetAlertDialogUtil.showDialog(errSweetAlertDialog, msg, PenalizeRecordListActivity.this, dialog -> {
                            dialog.dismiss();
                            //跳转到登录页
                ...
                try {
                    PenalizeRecordListActivity.this.getThrowable(mThrowable);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            public void getErrorNews(String str) {
                ...
                try {
                    PenalizeRecordListActivity.this.getErrorNews(str);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public void onEventMainThread(String strRefresh) {
        if (strRefresh.equals(EventBusService.PENALIZE_RECORD_LIST_REFRESH)) {
            againRequest();
        }
    }
    ...
    private void showErrorView() {
        SwipeRefreshUtil.swipeRefreshLayoutCustom(mSwipeRefreshLayout, mCustomEmptyView, mRecyclerView, R.string.str_hint_hygl_cfjl);
    }
    ...
    private void againRequest() {
        try {
            ...
            mAdapter.notifyDataSetChanged();
            mMemberPresenter.getPenalizeRecordList(mHyUserDetailEntity.getBasicinfo().getMid(), pi, ps,mHyUserDetailEntity.getBasicinfo().getXhuid(),type);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void initRecyclerView() {
        mAdapter = new PenalizeRecordAdapter(null, mHyUserDetailEntity, mMemberPresenter,type);
