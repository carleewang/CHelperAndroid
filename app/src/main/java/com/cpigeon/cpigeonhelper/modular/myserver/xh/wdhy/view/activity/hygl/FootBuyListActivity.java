...
public class FootBuyListActivity extends ToolbarBaseActivity {
    ...
    protected void setStatusBar() {
        ...
        setTopLeftButton(R.drawable.ic_back, FootBuyListActivity.this::finish);
        setTitle("足环购买记录");
        ...
        if (type.equals("myself") && !mHyUserDetailEntity.getZhuangtai().equals("除名")) {
            setTopRightButton(R.mipmap.top_add, () -> {
                //足环购买添加
                Intent intent = new Intent(FootBuyListActivity.this, FootBuyAddActivity.class);
    ...
    public void onEventMainThread(String strRefresh) {
        if (strRefresh.equals(EventBusService.FOOT_BUY_LIST_REFRESH)) {
            againRequest();//刷新
        }
    }
                ...
                try {
                    ...
                    if (mThrowable != null) {//抛出异常
                        ...
                        if (listApiResponse.getErrorCode() == 0) {
                            ...
                            errSweetAlertDialog = SweetAlertDialogUtil.showDialog(errSweetAlertDialog, msg, FootBuyListActivity.this, dialog -> {
                                dialog.dismiss();
                                //跳转到登录页
    ...
    private void showErrorView() {
        SwipeRefreshUtil.swipeRefreshLayoutCustom(mSwipeRefreshLayout, mCustomEmptyView, mRecyclerView, R.string.str_hint_hygl_zhgm);
    }
    ...
    private void againRequest() {
        try {
            ...
            mAdapter.notifyDataSetChanged();
            mMemberPresenter.getFootBuyEntityList(mHyUserDetailEntity.getBasicinfo().getMid(), pi, ps,mHyUserDetailEntity.getBasicinfo().getXhuid(),type);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void initRecyclerView() {
        mAdapter = new FootBuyAdapter(null, mHyUserDetailEntity,type);
